...
| `handlers/<Contract>Handler.sol` | Step 5 + Step 7 | Per-contract handler file in `{SUITE_DIR}`. Stub handler templates are generated in Step 5 from `ai_fuzzing_data/entry-point-selection.json`, then replaced or rewritten in Step 7. |
...
| File | Phase | Role |
|---|---|---|
| `ai_fuzzing_data/contracts.json` | Step 2 | Extracted contract/function inventory used as the structural input for selection and later analysis. |
| `ai_fuzzing_data/entry-point-selection.json` | Step 4 | Auto-accepted selection of contracts and functions that limits handler generation only. |
| `ai_fuzzing_data/protocol-understanding.md` | Step 3 fallback | Persisted protocol understanding notes when `x-ray` is unavailable or insufficient. |
