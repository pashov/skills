---
name: fuzz-sync
description: Reconcile an existing ai-fuzzing harness with a changed source tree. Detects added/removed/changed contract functions, quarantines stale properties, regenerates drifted handler stubs, and refreshes the snapshot. Trigger on "fuzz-sync", "resync fuzzing", "sync fuzz harness", "refresh fuzzing properties", "fuzzing drift check".
---

# Fuzz Sync Skill — fuzz-sync

Reconcile a project previously processed by the **ai-fuzzing** skill with a changed source tree. This is the re-use entry point: after the user modifies Solidity sources, `fuzz-sync` detects drift, quarantines stale properties, regenerates drifted handler stubs, and refreshes the snapshot — WITHOUT re-running the full 11-step pipeline.

## When to use

- The source contracts under `src/` changed since `ai-fuzzing` last ran.
...
- `SUITE_DIR`: Solidity suite directory relative to project root (default: `test/ai_fuzzing`). Use the same value that was passed to the `ai-fuzzing` skill when the harness was generated.
- `META_DIR`: Metadata directory relative to project root (default: `ai_fuzzing_data`). Use the same value that was passed to the `ai-fuzzing` skill.

## Arguments

- `--init` — one-time bootstrap: create `{META_DIR}/last-run.json` from the current state. Use this the first time fuzz-sync runs on a project (or when adopting fuzz-sync on a project that was fuzzed before the snapshot format existed).
- `--only <Contract>` — scope the sync to a single contract name.
- `--apply` — actually perform the automatic fixes. Without `--apply`, fuzz-sync runs in dry-run mode and only reports drift.
...
- The project must have been processed by the `ai-fuzzing` skill at least once. Look for:
...
3. **Do NOT re-run `select_functions.js` automatically.** The existing `entry-point-selection.json` encodes the user's prior tier assignments and selection choices. fuzz-sync treats it as the source of truth for what's "in scope" and diffs only within that scope.

   If new contracts have been added that the user probably wants to fuzz, the drift report will surface them and the user can manually add them to `entry-point-selection.json` and re-run fuzz-sync.
...
1. Run:
   ...
   ```
   node {SKILL_PATH}/../../scripts/fuzz_sync.js {PROJECT_ROOT} --init
   ```
...
```
node {SKILL_PATH}/../../scripts/fuzz_sync.js {PROJECT_ROOT}
```

The script:
- Reads `{META_DIR}/last-run.json` (created by `--init` or by Step 11 of the main `ai-fuzzing` skill).
...
If fuzz-sync was invoked WITHOUT `--apply`, stop here and tell the user:
...
```
node {SKILL_PATH}/../../scripts/fuzz_sync.js {PROJECT_ROOT} --apply-handlers
```
...
3. Keep `/// @notice SP-NN:` doctags intact — they are how `fuzz-convert` and future `fuzz-sync` runs identify which Solidity corresponds to which Spec ID.
...
```
node {SKILL_PATH}/../../scripts/fuzz_sync.js {PROJECT_ROOT} --refresh-snapshot
```

This overwrites `{META_DIR}/last-run.json` with the post-sync state. Future `fuzz-sync` runs will diff against this new baseline.
...
- For newly added functions without handler bodies: re-run `ai-fuzzing` Step 7 for just those contracts, or use the `fuzz-convert` skill if the user also wants to regenerate properties for them.
- For quarantined properties: either rewrite them manually (flip `[~]` back to `[ ]` in `PROPERTIES.md` and run `/fuzz-convert`), or leave them quarantined.
...
- Never run a full ai-fuzzing pipeline as part of a sync. Sync is surgical by design.
