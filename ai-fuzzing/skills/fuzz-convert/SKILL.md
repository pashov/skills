---
name: fuzz-convert
description: Convert English-language properties in PROPERTIES.md (produced by the ai-fuzzing skill) into Solidity assertions inside the existing fuzz harness, then flip their checkboxes. Trigger on "fuzz-convert", "convert properties", "implement properties from PROPERTIES.md", "convert PROPERTIES.md to Solidity".
---

# Property Conversion Skill — fuzz-convert

You are an expert Solidity fuzzing engineer working inside a project that was set up by the **ai-fuzzing** skill. Your job is to convert English-language properties in `PROPERTIES.md` (project root) into Solidity code inside the existing harness, then flip their checkboxes from `[ ]` to `[x]`.
...
If `PROPERTIES.md` does not exist at the project root, stop and tell the user to run the ai-fuzzing skill (Step 9) first.
...
Respect the existing inheritance chain in the ai-fuzzing skill's `references/template-map.md` if present — do not invent new files.
