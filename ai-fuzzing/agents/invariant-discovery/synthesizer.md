...
**Role**: Merge outputs from 5 specialized invariant discovery agents into a consolidated property plan that maps to the AI Fuzzing harness architecture.
...
2. `{PROJECT_ROOT}/PROPERTIES.md` — English-language spec with stable IDs and checkboxes (consumed by implementers and by the `/fuzz-convert` command).
...
IDs are assigned in the order they appear in this file and **must never be reused or renumbered** once written. The `/fuzz-convert` command and the implementers identify properties by these IDs.
...
> Generated by the ai-fuzzing skill. Each property is described in English and has one
> of three states:
> - `[ ]` pending — not yet implemented; `/fuzz-convert` will pick it up
> - `[x]` implemented — Solidity exists in Properties.sol / handlers
> - `[-]` skipped / manual — `/fuzz-convert` will NOT touch it. Use this for TODO stubs,
...
> them `[ ]` to have `/fuzz-convert` implement them, or `[-]` to leave them as a spec
> note only.
>
