---
name: ai-fuzzing
description: Generate Echidna/Medusa-compatible Solidity fuzz suites from Foundry or Hardhat projects. Trigger on "ai-fuzzing", "generate fuzz suite", "build fuzz harness", "stateful fuzzing", "fuzzing harness", "property testing", and "invariant suite".
---

# AI Fuzzing

Generate a stateful Solidity fuzz suite under `{SUITE_DIR}` (default: `test/ai_fuzzing/`), with metadata and fuzzer runtime files under `{META_DIR}` (default: `ai_fuzzing_data/`).
...
- Keep the generated Solidity suite isolated under `test/ai_fuzzing/` and the metadata/runtime files under `ai_fuzzing_data/` unless the user explicitly asks for different paths.
...
- `SUITE_DIR`: `test/ai_fuzzing` relative to `PROJECT_ROOT`. Pass `--suite-dir` to suite-generation steps.
- `META_DIR`: `ai_fuzzing_data` relative to `PROJECT_ROOT`. Pass `--meta-dir` to metadata steps.
...
**Version check** (foreground — do this first, after the banner and mode resolution):

- Read the local `VERSION` file from `{SKILL_PATH}/VERSION`.
- Bash `curl -sf https://raw.githubusercontent.com/pashov/skills/main/ai-fuzzing/VERSION`.
- If the remote VERSION fetch succeeds and differs from local, print `⚠️ You are not using the latest version. Please upgrade for best fuzzing coverage. See https://github.com/pashov/skills`. If it fails, skip silently.
...
- `{PROJECT_ROOT}/PROPERTIES.md` — English-language spec with `[ ]` checkboxes, one entry per property, identified by the same Spec IDs. This is the artifact that the `/fuzz-convert` command and the implementers in Step 9d operate on.
...
Both implementers MUST flip `[ ]` → `[x]` in `{PROJECT_ROOT}/PROPERTIES.md` for each property they actually implement (matching by Spec ID `GL-NN` / `SP-NN`). Properties left as TODO stubs stay `[ ]` so `/fuzz-convert` can pick them up later.
...
After the final report is written, capture a sync snapshot so the `/fuzz-sync` skill can detect drift on subsequent source changes without re-running the full pipeline:

```
node {SKILL_PATH}/scripts/fuzz_sync.js {PROJECT_ROOT} --init --meta-dir {META_DIR} --suite-dir {SUITE_DIR}
```
...
```
node {SKILL_PATH}/scripts/fuzz_sync.js {PROJECT_ROOT} --refresh-snapshot --meta-dir {META_DIR} --suite-dir {SUITE_DIR}
```

This writes `{PROJECT_ROOT}/{META_DIR}/last-run.json` — a hash+signature snapshot of the in-scope contracts, handlers, and `PROPERTIES.md` entries. Later, when the user modifies sources and invokes `/fuzz-sync`, that skill diffs against this file to detect added/removed/changed functions, quarantine stale properties, and regenerate only the drifted handler stubs.
