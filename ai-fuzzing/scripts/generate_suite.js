 ...
 *
 * Defaults:
 *   --suite-dir    test/ai_fuzzing  (relative to project root)
 *   --meta-dir     ai_fuzzing_data  (relative to project root)
...
let suiteRelDir = path.join('test', 'ai_fuzzing');
let metaRelDir = 'ai_fuzzing_data';
    ...
    for (const entry of fs.readdirSync(src, { withFileTypes: true })) {
        ...
        if (entry.isDirectory()) {
            ...
            if (fs.existsSync(dstPath)) {
                ...
                const needsSubstitution = metaRelDir !== 'ai_fuzzing_data' && entry.name === 'README.md';
                if (needsSubstitution) {
                    const content = fs.readFileSync(srcPath, 'utf8').replaceAll('ai_fuzzing_data', metaRelDir);
...
function copyRootConfigs() {
    for (const filename of ['echidna.yaml', 'medusa.json']) {
        ...
        if (fs.existsSync(dst)) {
            ...
            if (metaRelDir !== 'ai_fuzzing_data') {
                content = content.replaceAll('ai_fuzzing_data', metaRelDir);
            }
