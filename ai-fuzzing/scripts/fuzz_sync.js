 ...
 * ai-fuzzing skill. Compares the stored snapshot (ai_fuzzing_data/last-run.json)
 * against the current source/ABI state and emits:
 *   - a human-readable report to stdout
 *   - a machine-readable JSON report for consumption by the /fuzz-sync skill
 *
 * Modes:
 ...
 *   --meta-dir <dir>   Metadata directory (default: ai_fuzzing_data).
 *   --suite-dir <dir>  Suite directory    (default: test/ai_fuzzing).
...
const opts = {
    ...
    force: false,
    only: null,
    metaRelDir: 'ai_fuzzing_data',
    suiteRelDir: path.join('test', 'ai_fuzzing'),
    reportPath: null,
};
...
function printUsage() {
    console.log(
`Usage: node fuzz_sync.js <PROJECT_ROOT> [mode] [options]
...
function buildSnapshot() {
    ...
    if (!selection) fail(2, `missing ${path.relative(projectRoot, selectionPath)} — run Step 4 of the ai-fuzzing skill first.`);
...
function printReport(report) {
    const lines = [];
    lines.push('');
    lines.push('╭────────────────── fuzz-sync drift report ──────────────────╮');
    if (!report.hasDrift) {
...
function applyHandlerRegen(report) {
    ...
    if (driftedNames.size === 0) {
        console.log('fuzz-sync: no added/changed contracts to regenerate — nothing to do.');
    ...
    console.log(`fuzz-sync: running ${cmd}`);
...
function main() {
    ...
    if (!fs.existsSync(metaDir))     fail(2, `meta dir not found: ${opts.metaRelDir} — has the ai-fuzzing skill been run?`);
    ...
    if (opts.init) {
        ...
        console.log(`fuzz-sync: wrote initial snapshot to ${path.relative(projectRoot, snapshotPath)} (${snap.contracts.length} contracts, ${snap.properties.length} properties).`);
        return;
    }

    const oldSnap = loadJson(snapshotPath);
    if (!oldSnap) fail(2, `no snapshot at ${path.relative(projectRoot, snapshotPath)} — run: node fuzz_sync.js ${opts.metaRelDir === 'ai_fuzzing_data' ? '.' : projectRoot} --init`);

    if (opts.refreshSnapshot) {
        const snap = buildSnapshot();
        fs.writeFileSync(snapshotPath, JSON.stringify(snap, null, 2));
        console.log(`fuzz-sync: refreshed snapshot at ${path.relative(projectRoot, snapshotPath)} (${snap.contracts.length} contracts, ${snap.properties.length} properties).`);
        return;
    }
    ...
    if (opts.applyHandlers) {
        ...
        console.log(`fuzz-sync: regenerated ${result.regenerated.length} handler(s); backups: ${result.backedUp.length}`);
...
try {
    ...
    console.error(`fuzz-sync: unexpected error: ${e.stack || e.message}`);
    process.exit(3);
}
