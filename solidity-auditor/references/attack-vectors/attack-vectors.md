...
**52. False Completion Without Asset Movement**

- **D:** Public or weakly-gated function advances distribution, settlement, refund, fill, repayment, or completion state from requested amount, rounded amount, or counters while actual asset transfer is zero or partial. A payer/maker/distributor using a second address can finalize the obligation and retain inventory or avoid payment.
- **FP:** Terminal state is keyed to actual token/ETH/NFT deltas or post-transfer balance reconciliation. Zero/partial transfers cannot consume the full obligation, and retry/recovery remains open until full value movement is proven.
